...
class StatsDB:
            ...
            for model_name, pricing in pricing_models.items():
                ...
                if cached_input_price is not None:
                    ...
                    query = '''
                        UPDATE requests
                        SET
                            input_cost = ((input_tokens - COALESCE(cached_tokens, 0)) * ?) / ?,
                            cached_cost = (COALESCE(cached_tokens, 0) * ?) / ?,
                            output_cost = (output_tokens * ?) / ?,
                            total_cost = ((input_tokens - COALESCE(cached_tokens, 0)) * ? + COALESCE(cached_tokens, 0) * ? + output_tokens * ?) / ?,
                            currency = ?
                        WHERE model = ?
                    '''
                    cursor.execute(query, (
                        input_price, unit,
                        cached_input_price, unit,
                        output_price, unit,
                        input_price, cached_input_price, output_price, unit,
                        model_currency,
                        model_name
                    ))
                else:
                    # 未配置缓存价格：全部输入token合并计算，缓存成本为0
                    query = '''
                        UPDATE requests
                        SET
                ...
                if model_currency == 'CNY':
                    model_total_usd = model_total * CNY_TO_USD
    ...
    # ==================== Async 包装方法 ====================
    # 🔧 性能修复：将同步 SQLite 查询包裹在 asyncio.to_thread() 中
    # 避免阻塞 asyncio 事件循环，允许多个查询真正并发执行

    async def get_token_stats_async(self, start_time=None, end_time=None, model_config=None, rpm_period=None):
        """异步版本的 get_token_stats，不阻塞事件循环"""
    ...
    async def get_request_stats_async(self, start_time=None, end_time=None):
        """异步版本的 get_request_stats，不阻塞事件循环"""
    ...
    async def get_request_summary_async(self, start_time=None, end_time=None):
        """异步版本的 get_request_summary，不阻塞事件循环"""
