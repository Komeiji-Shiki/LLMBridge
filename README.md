...
最近的[监控检索与 Playground 优化](docs/OPTIMIZATION_2026-09-10.md)新增日志日期筛选、自动刷新暂停和明确的加载反馈；请求调试支持端点草稿保留、工具参数保留以及跳转到对应日志。

本轮审查与实施结果见 [全项目实施总报告](docs/GATEWAY_IMPLEMENTATION_REPORT_2026-09-05.md)。管理面板新增“能力与 Playground”，支持供应商原生工具、原生 JSON 请求、阶段耗时、调用方统计和独立的现价分析。完整请求归档永久保留，会话缓存闲置三天清理，临时图床保留三十分钟；配置和调用示例见报告。

> ⚠️ **维护状态说明**
>
> **LMArena 功能已经不再维护**（油猴脚本桥接、Battle 模式、会话 ID 捕获、自动抓取模型等），无需考虑相关代码的维护与修复。
> 目前项目重点是**中转 API**（Direct API：OpenAI 兼容透传 / Anthropic 原生 / Gemini 原生转发）。
...
- **模型归档**：一键归档/恢复模型（归档后无法请求、不出现在模型列表、管理面板折叠展示）；支持按最后调用时间自动归档闲置模型（`config.jsonc` 的 `auto_archive` 配置，默认关闭）
