...
async function refreshRequestLogs() {
    ...
    try {
        ...
        tbody.innerHTML = logs.map(log => {
            ...
            return `
                <tr>
                    <td><input type="checkbox" class="compare-check" data-request-id="${escapeHtml(log.request_id || '')}" onchange="toggleCompareSelection(this.dataset.requestId, this.checked)" ${log.request_id && compareSelection.includes(log.request_id) ? 'checked' : ''} ${log.request_id ? '' : 'disabled title="该记录无请求ID"'}></td>
                    <td>${time}</td>
    ...
    try {
        ...
        modalBody.innerHTML = `
            <div class="detail-section detail-overview">
                ...
                <div class="detail-item">
                    <div class="detail-label">生成速度:</div>
                    <div class="detail-value">${renderTpsRowHtml(computeTps(details))}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">缓存对比:</div>
                    <div class="detail-value"><button class="detail-btn" onclick="compareWithCurrent('${escapeHtml(details.request_id || '')}')">⧉ 加入对比选择</button> <span id="compare-current-hint" style="font-size:12px;opacity:.7;">已选 ${compareSelection.length}/2</span></div>
                </div>
                <div class="detail-item">
...
// 解码速度 = 输出tokens / 输出阶段秒（timings.output_ms），端到端 = 输出tokens / duration
function computeTps(details) {
    ...
    const outputS = outputMsRaw > 0 ? outputMsRaw / 1000 : null;
    const ttftMsRaw = Number(timings.first_business_ms);
    const ttftS = ttftMsRaw > 0 ? ttftMsRaw / 1000 : null;
    ...
    const caveat = (decodeTps != null && duration && outputS && outputS < duration / 10)
        ? '输出阶段远短于总耗时（多为非流式响应），解码速度仅供参考，以端到端速度为准。'
        : null;
...
function renderTpsRowHtml(tps) {
    ...
    const parts = [];
    if (tps.decodeTps != null) parts.push(`解码 <strong>${escapeHtml(tps.decodeTps.toFixed(1))}</strong> tok/s`);
    ...
    if (tps.ttftS != null) meta.push(`TTFT ${tps.ttftS.toFixed(2)}s`);
...
function updateCompareButton() {
    ...
    if (hint) hint.textContent = compareSelection.length === 2 ? '已选两条，点击对比' : '';
...
function toggleCompareSelection(requestId, checked) {
    ...
    if (checked) {
        compareSelection.push(requestId);
        if (compareSelection.length > 2) {
            const removed = compareSelection.shift();
            document.querySelectorAll(`.compare-check[data-request-id="${CSS.escape(removed)}"]`).forEach(box => { box.checked = false; });
        }
    }
    updateCompareButton();
}
...
function toggleCompareSelectAll(master) {
    ...
    if (master && master.checked) {
        boxes.slice(0, 2).forEach(box => {
            box.checked = true;
            if (box.dataset.requestId) compareSelection.push(box.dataset.requestId);
        });
        const hint = document.getElementById('compare-hint');
        if (hint && boxes.length > 2) hint.textContent = `本页 ${boxes.length} 条，仅勾选前 2 条`;
    }
    updateCompareButton();
    if (master) master.checked = false;
}
...
// 详情页“加入对比选择”：满 2 条时替换最早的一条
function compareWithCurrent(requestId) {
    if (!requestId) return;
    if (!compareSelection.includes(requestId)) {
        compareSelection.push(requestId);
        if (compareSelection.length > 2) compareSelection.shift();
    }
    document.querySelectorAll(`.compare-check[data-request-id="${CSS.escape(requestId)}"]`).forEach(box => { box.checked = true; });
...
function renderDiffContext(text, absOffset) {
    ...
    return escapeHtml(text.slice(0, rel)) +
        '<span class="diff-mark" title="首个差异位置">⇄</span>' +
        escapeHtml(text.slice(rel));
}

function renderTpsMini(label, tps) {
    if (!tps) return '';
    return `<div class="compare-tps"><span>${escapeHtml(label)}</span>` +
        `<strong>解码 ${escapeHtml(tps.decode_tps != null ? tps.decode_tps.toFixed(1) : '—')}</strong>` +
        `<strong>端到端 ${escapeHtml(tps.e2e_tps != null ? tps.e2e_tps.toFixed(1) : '—')}</strong>` +
        `<span>tok/s${tps.ttft_s != null ? ` · TTFT ${escapeHtml(String(tps.ttft_s))}s` : ''}</span></div>`;
}
...
function renderCompareResult(data) {
    ...
    let messagesHtml;
    if (!first) {
        messagesHtml = `<div class="compare-ok">✅ 公共部分的 ${escapeHtml(String(messages.common_count ?? 0))} 条消息逐字节一致。` +
            (messages.strict_append
                ? `新请求是旧请求 history 的严格追加（多出 ${escapeHtml(String((messages.appended || []).length))} 条）。`
                : '消息数量不一致但公共部分一致。') + '</div>';
    } else {
    ...
    if (tools.status === 'identical') {
        ...
        toolsHtml = '<div class="compare-bad">💥 工具定义不一致——工具是 prompt 前缀的一部分，任何改动都会破坏缓存。</div>' +
        ...
        toolsHtml = '<div class="compare-warn">❓ 日志未记录工具定义（旧日志或 responses 原生透传仅记摘要），不能排除工具变化导致缓存断裂。</div>';
    }

    let paramsHtml;
    if (params.same) {
        paramsHtml = '<div class="compare-ok">✅ 其余缓存相关参数全部一致。</div>';
    } else {
        paramsHtml = '<div class="compare-bad">💥 以下缓存相关参数发生变化：</div><div class="table-container"><table class="compare-table"><thead><tr><th>参数</th><th>旧 (A)</th><th>新 (B)</th></tr></thead><tbody>' +
    ...
    if (!identityFields.length) {
        identityHtml = '<div class="compare-ok">两条日志均未记录会话/调用方归属字段。</div>';
    } else if (identity.same) {
        identityHtml = `<div class="compare-ok">✅ 同一归属（${escapeHtml(identityFields.map(f => `${f.key}=${f.old}`).join(' · ') || '—')}），缓存空间一致，掉命中值得深究。</div>`;
    } else {
        identityHtml = '<div class="compare-warn">❓ 归属不一致——缓存空间通常按会话/调用方隔离，两条请求可能本来就不共享缓存。</div><div class="table-container"><table class="compare-table"><thead><tr><th>归属字段</th><th>旧 (A)</th><th>新 (B)</th></tr></thead><tbody>' +
    ...
    body.innerHTML = `
        ...
        <div class="detail-section">
            ...
            <div class="detail-item"><div class="detail-label">对齐分析:</div><div class="detail-value">两命中数 GCD=${escapeHtml(String(summary.cache_gcd ?? '—'))}${summary.both_divisible_by_128 ? '，均为 128 的倍数（符合块对齐）' : '，不全是 128 的倍数'}${summary.start_to_start_gap_s != null ? `；开始间隔 ${escapeHtml(String(summary.start_to_start_gap_s))}s` : ''}${summary.prev_end_to_new_start_gap_s != null ? `；上条结束到本条开始 ${escapeHtml(String(summary.prev_end_to_new_start_gap_s))}s` : ''}</div></div>
        ...
        <div class="detail-section"><h3>结论</h3><ul class="compare-inference">${inferenceHtml}</ul></div>
    `;
}
...
// 保持键盘焦点在详情/对比窗口内，关闭后回到原入口。
document.addEventListener('keydown', event => {
    const detail = document.getElementById('detailModal');
    const compare = document.getElementById('compareModal');
    const modal = detail.style.display === 'block' ? detail
        : (compare && compare.style.display === 'block' ? compare : null);
    if (!modal) return;
    if (event.key === 'Escape') {
        if (modal.id === 'compareModal') closeCompareModal(); else closeModal();
        return;
    }
...
window.onclick = function(event) {
    ...
    const compare = document.getElementById('compareModal');
    if (compare && event.target == compare) {
        closeCompareModal();
    }
}
