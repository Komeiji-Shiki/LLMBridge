...
function switchTab(tab) {
    ...
    document.getElementById('compare-toolbar').style.display = tab === 'requests' ? 'flex' : 'none';
...
async function viewRequestDetails(requestId) {
    ...
    try {
        const response = await apiGet(`/api/request/${encodeURIComponent(requestId)}`);
        ...
        modalBody.innerHTML = `
            <div class="detail-section detail-overview">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value">${MonitorCompare.renderTpsRowHtml(MonitorCompare.computeTps(details))}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">缓存对比:</div>
                    <div class="detail-value"><button class="detail-btn" data-request-id="${escapeHtml(details.request_id || '')}" onclick="MonitorCompare.compareWithCurrent(this.dataset.requestId)">⧉ 加入对比选择</button> <span id="compare-current-hint" style="font-size:12px;opacity:.7;">已选 ${MonitorCompare.count}/2</span></div>
                </div>
                <div class="detail-item">
...
function closeModal() {
    ...
    if (document.getElementById('compareModal').style.display !== 'block') document.body.classList.remove('detail-open');
...
document.addEventListener('keydown', event => {
    ...
    const modal = compare?.style.display === 'block' ? compare
        : (detail.style.display === 'block' ? detail : null);
    if (!modal) return;
    if (event.key === 'Escape') {
        if (modal.id === 'compareModal') MonitorCompare.closeCompareModal(); else closeModal();
        return;
    }
    if (event.key !== 'Tab') return;
    const focusable = [...modal.querySelectorAll('button, a[href], summary, input, select, textarea, [tabindex="0"]')].filter(el => !el.disabled && el.getClientRects().length);
